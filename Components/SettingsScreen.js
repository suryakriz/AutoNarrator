...
class SettingsScreen extends Component {
  ...
  async speaktest() {
    var thingToSay =
      ...
      " at the speed " +
      this.state.rate +
      ". If you do not like it feel free to change the settings.";
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={{ fontFamily: "Quicksand-Regular", color: "#ffffff" }}>
            {" "}
            Voice Settings{" "}
          </Text>
        </View>
        ...
        <View style={styles.settings}>
          ...
          <DropDownPicker
            ...
            dropDownStyle={{ backgroundColor: "#fafafa", elevation: 999 }}
            labelStyle={{ fontFamily: "Quicksand-Regular" }}
            onChangeItem={(item) => this.ChangeTimeBetween(item)}
          />
        </View>
        <View style={styles.settings}>
          <AwesomeButtonBlue
  ...
  ChangeVoice(newVoice) {
    console.log("\n Changing voice to:", newVoice.label);
    this.setState({
      voice: newVoice.value,
      label: newVoice.label,
    });
    this.props.dispatch(
      SetVoice({ voice: newVoice.value, label: newVoice.label })
    );
  }

  ChangeSpeed(rate) {
    console.log("\n Changing rate to:", rate);
    this.setState({ rate: rate });
    this.props.dispatch(SetSpeed(rate));
  }

  ChangeTimeBetween(rate) {
    console.log("\n Changing time between to:", rate.value);
    this.setState({
      time: rate.value,
    });
